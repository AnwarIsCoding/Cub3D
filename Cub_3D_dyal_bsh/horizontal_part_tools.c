...
/*   Updated: 2025/10/28 14:37:00 by amaliari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	for_horizontal_intersection(t_intercept *intercept, t_ray *rays, int strip_id, t_game *cub)
{
	...
	intercept->x_intercept = cub->player->x + (intercept->y_intercept - cub->player->y)
...
void	affecting_to_hori_struct(t_hori *hori, t_game *cub, int grid_x, int grid_y)
{
	hori->hori_wall_hit_x = hori->next_hori_touch_x;
    hori->hori_wall_hit_y = hori->next_hori_touch_y;
    hori->hori_wall_content = cub->game_info->map[grid_y][grid_x];
...
void	find_horizontal_wall_hit(t_hori *hori, t_step steps, t_game *cub,  int strip_id)
{
	...
	while (hori->next_hori_touch_x >= 0 && hori->next_hori_touch_x <= cub->game_info->num_rows * TILE_SIZE
        && hori->next_hori_touch_y >= 0 && hori->next_hori_touch_y <= cub->game_info->num_cols * TILE_SIZE)
    {
        ...
        if (wall_collision(cub, x_to_check, y_to_check))             
        {
            ...
            if (grid_x < 0 || grid_x >= cub->game_info->num_cols || grid_y < 0 || grid_y >= cub->game_info->num_rows)
				break;
            affecting_to_hori_struct(hori, cub, grid_x, grid_y);
            break;
        }
