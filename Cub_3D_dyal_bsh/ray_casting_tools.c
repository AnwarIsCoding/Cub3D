...
/*   Updated: 2025/10/26 16:47:47 by amaliari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_hori  *horizontal_part(t_intercept intercept, t_step steps, t_game *cub, int strip_id)
{
    ...
    initializing_horizontal_part(hori, intercept.x_intercept, intercept.y_intercept);
	find_horizontal_wall_hit(hori, steps, cub, strip_id);
    if (hori->hori_wall_hit)
    hori->hori_hit_distance = distance_between_points(cub->player->x, cub->player->y, 
                                                   hori->hori_wall_hit_x, 
                                                   hori->hori_wall_hit_y);    
    else 
...
t_vert  *vertical_part(t_intercept intercept, t_step steps, t_game *cub, int strip_id)
{
    ...
    initializing_vertical_part(vert, intercept.x_intercept, intercept.y_intercept);
	find_vertical_wall_hit(vert, steps, cub, strip_id);
	if (vert->vert_wall_hit)
    vert->vert_hit_distance = distance_between_points(cub->player->x, cub->player->y, 
